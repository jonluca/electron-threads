...
export const Thread = {
  ...
  errors<ThreadT extends ThreadType>(thread: ThreadT): Observable<Error> {
    return (
      thread[$errors] ||
      fail("Error observable not found. Make sure to pass a thread instance as returned by the spawn() promise.")
    );
  },
  ...
  events<ThreadT extends ThreadType>(thread: ThreadT): Observable<WorkerEvent> {
    return (
      thread[$events] ||
      fail("Events observable not found. Make sure to pass a thread instance as returned by the spawn() promise.")
    );
  },
